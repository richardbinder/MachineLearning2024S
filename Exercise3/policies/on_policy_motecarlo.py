...
def on_policy_monte_carlo(
    ...
    # Create a new matrix C with the same shape as Q, where each element is an empty list
    C = np.zeros(Q.shape + (2,), dtype=object)
    ...
    for i in range(total_episodes):
        ...
        while T:
            ...
            C[position][action][0] += G
            C[position][action][1] += 1
            Q[position][action] = C[position][action][0] / C[position][action][1]
